...
export const createUserProject = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (user && user.credits < 5) {
            return res.status(403).json({ success: false, message: "You do not have enough credits to create website" })
        }
...
export const getSingleProject = async (req: Request, res: Response) => {
    try {
        ...
        if (!projectId) {
            return res.status(400).json({ success: false, message: "Please provide projectId" })
        }
        const project = await prisma.websiteProject.findUnique({
            where: { userId, id: projectId },
            include: { conversation: { orderBy: { timestamp: "asc" } }, versions: { orderBy: { timestamp: "asc" } } }
        })
...
export const toggleProjectPublish = async (req: Request, res: Response) => {
    try {
        ...
        res.status(200).json({ success: true, message: project.isPublished ? "Project Unpublished" : "Project Published Successfully" })
    } catch (error: any) {
        console.log(error)
        return res.status(500).json({ success: false, message: error.message })
    }
